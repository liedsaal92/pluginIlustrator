...
function generatePrintHtml(entry: TablaExportEntry): string {
  ...
  const dateTag = new Date(entry.createdAt)
    ...
    .replace(/\//g, '');
  const docTitle = `TablaDePreciosPara${safeName}_${dateTag}`;
...
// ── Historial: fila simplificada (sin expandir) ────────────
...
export function TablaClienteScreen({ onToast }: Props) {
  const {
    config, basePrices, supplies, machines, operations, volumeTiers, printProfiles,
  ...
  // Rows computed AFTER paint to keep selects responsive
  ...
  // Compute rows after render so selects stay responsive
  useEffect(() => {
    const rows: LiveRow[] = [];
        ...
        for (const { id: productId } of PRODUCTS) {
          try {
            const input: QuoteInput = {
              ...
              savingsTransferRate: transferRate, config, tallaDims,
            };
  ...
  }, [segment, profileId, basePrices, supplies, machines, operations, volumeTiers, transferRate, config, refClienteId, refGender, tallasPorCliente]);

  const liveHRows = liveRows.filter(r => r.gender === 'H');
  const liveMRows = liveRows.filter(r => r.gender === 'M');
  ...
  function handleExport() {
    const rows: TablaExportRow[] = liveRows.map(({ size, gender, prices }) => ({ size, gender, prices }));
  ...
  return (
    <div className="screen pricing-screen">
      ...
      <div className="pricing-header">
        <div>
          <h1 className="pricing-title">TABLA DE PRECIOS</h1>
          <div className="pricing-subtitle">// Lista de precios personalizada para tu cliente</div>
        </div>
        {activeTab === 'tabla' && (
      ...
      {/* ── Tabs ──────────────────────────────────────────────── */}
      <div className="pricing-transfer-btns" style={{ marginTop: '1rem', maxWidth: '320px' }}>
        <button
          className={`pricing-transfer-btn${activeTab === 'tabla' ? ' active' : ''}`}
          ...
          <div className="tabla-cliente-printable no-print" style={{ marginTop: '1.5rem' }}>
            <div className="tabla-cliente-print-header">
              <div className="tabla-cliente-brand-block">
                ...
                {clienteNombre
                  ? <div className="tabla-cliente-recipient">Para: <strong>{clienteNombre}</strong></div>
            ...
            <LiveTableSection rows={liveHRows} showCosto={showCosto} />
            <LiveTableSection rows={liveMRows} showCosto={showCosto} />
      ...
      {activeTab === 'historial' && (
        <section className="pricing-panel" style={{ marginTop: '1rem', padding: '1.25rem' }}>
          {tablaExports.length === 0 ? (
            <div className="pricing-table-sub">Sin exportaciones aún. Generá tu primera tabla desde NUEVA TABLA.</div>
          ) : (
            <>
